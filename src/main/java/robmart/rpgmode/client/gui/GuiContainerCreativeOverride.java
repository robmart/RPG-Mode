...
public class GuiContainerCreativeOverride extends InventoryEffectRenderer
{
        ...
        else
        {
            ...
            else if (selectedTabIndex == CreativeTabs.INVENTORY.getTabIndex())
            {
                ...
                else if (type == ClickType.THROW && slotIn != null && slotIn.getHasStack())
                {
                    ...
                    this.mc.playerController.sendSlotPacket(itemstack1, ((GuiContainerCreativeOverride.CreativeSlot)slotIn).slot.slotNumber);
                }
                ...
                {
                    this.mc.player.inventoryContainer.slotClick(slotIn == null ? slotId : ((GuiContainerCreativeOverride.CreativeSlot)slotIn).slot.slotNumber, mouseButton, type, this.mc.player);
    ...
    public void handleMouseInput() throws IOException
    {
        ...
        {
            int j = (((GuiContainerCreativeOverride.ContainerCreative)this.inventorySlots).itemList.size() + 9 - 1) / 9 - 5;
    ...
    public static class ContainerCreative extends Container
    {
        ...
        public ContainerCreative(EntityPlayer player)
        {
            ...
            for (int i = 0; i < 5; ++i)
            {
                for (int j = 0; j < 9; ++j)
                {
                    this.addSlotToContainer(new GuiContainerCreativeOverride.LockedSlot(GuiContainerCreativeOverride.basicInventory, i * 9 + j, 9 + j * 18, 18 + i * 18));
                }
            }
        ...
        public void scrollTo(float pos)
        {
            ...
            for (int k = 0; k < 5; ++k)
            {
                for (int l = 0; l < 9; ++l)
                {
                    ...
                    {
                        GuiContainerCreativeOverride.basicInventory.setInventorySlotContents(l + k * 9, this.itemList.get(i1));
                    }
                    else
                    {
                        GuiContainerCreativeOverride.basicInventory.setInventorySlotContents(l + k * 9, ItemStack.EMPTY);
                    }
                }
            }
        }
